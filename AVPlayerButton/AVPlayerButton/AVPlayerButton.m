...
@interface AVPlayerButton ()<CAAnimationDelegate>
{
...
#pragma mark -添加layer
- (void)setupUI {
    _buttonState = AVPlayerButtonStatePause;
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    ...
    BOOL isTriangle = [name isEqualToString:TriangleAnimationName];
    BOOL isRightLine = [name isEqualToString:RightLineAnimationName];
    if (isRightLine && _buttonState == AVPlayerButtonStatePlay) {
...
- (void)setButtonState:(AVPlayerButtonState)buttonState {
    if (_isAnimating) {
        return;
    }
    _buttonState = buttonState;
    if (buttonState == AVPlayerButtonStatePause) {
        //播放->暂停
        _isAnimating = YES;
    ...
    }   else if (buttonState == AVPlayerButtonStatePlay) {
        //暂停->播放
        _isAnimating = YES;
        //竖线正向动画
